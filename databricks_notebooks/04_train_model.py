...
with mlflow.start_run() as run:
    ...
    print("Model logged as an MLflow artifact on this run (retrievable via the MLflow API/UI).")

    # Unity Catalog blocks direct DBFS/artifact-proxy downloads in this workspace,
    # so also publish the model as base64 in a Delta table - a channel we know
    # works (same SQL Warehouse path used to read bronze/silver/gold). This is
    # what the local FastAPI service pulls from to serve predictions.
    import base64, json as _json, time as _time
    with open(model_path, "rb") as f:
        model_b64 = base64.b64encode(f.read()).decode("ascii")

    model_row = spark.createDataFrame([{
    ...
    model_row.write.format("delta").mode("overwrite").option("overwriteSchema", "true").saveAsTable("pl_predictor_gold.latest_model")
    print("Model published to pl_predictor_gold.latest_model")
